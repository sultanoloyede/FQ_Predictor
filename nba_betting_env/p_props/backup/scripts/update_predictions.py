...
def load_best_models(models_dir):
    ...
    """
    targets = ['10_plus', '15_plus', '20_plus']
    best_models = {}
...
def prepare_features(combined_data, target_threshold):
    ...
    for player_id, group in grouped:
        ...
        for window in [5, 10, 20]:
            past_games[f'ppg_last_{window}_games'] = past_games['points'].rolling(window=window, min_periods=window).mean()
            past_games[f'fga_avg_last_{window}_games'] = past_games['fga'].rolling(window=window, min_periods=window).mean()
        ...
        past_games['fga_avg_season'] = past_games['fga'].expanding(min_periods=20).mean()
        ...
        past_games['pct_10_plus_last_10_games'] = past_games['points'].rolling(window=10, min_periods=10).apply(lambda x: (x > 10).mean())
        past_games['pct_15_plus_last_10_games'] = past_games['points'].rolling(window=10, min_periods=10).apply(lambda x: (x > 15).mean())
        past_games['pct_20_plus_last_10_games'] = past_games['points'].rolling(window=10, min_periods=10).apply(lambda x: (x > 20).mean())

        # Compute average of bottom 20% scores up to the latest game
        past_games['avg_score_bottom_20_percent_games'] = past_games['points'].rolling(window=20, min_periods=20).apply(compute_bottom_20_avg)
        ...
        # >>> DEBUG: Log SGA’s feature row if player_id == 1628983 <<<
        if player_id == '1628983':  # Ensure it's the correct type (string or int)
            logging.debug("=== DEBUG (prepare_features) ===")
            logging.debug(f"SGA (player_id={player_id}) Feature Values:")
            logging.debug(latest_game[FEATURE_COLUMNS].to_dict())
    ...
    if features:
        ...
        logging.info(f'Prepared features for {players_processed} players for target > {target_threshold} points.')
...
def make_predictions(X, best_models):
    ...
    Returns a DataFrame with predictions and player information, including 'ppg_last_10_games'.
    """
    if X.empty:
        logging.error('Input features DataFrame is empty. Cannot make predictions.')
        return pd.DataFrame()

    # Start with 'player_id' and 'ppg_last_10_games'
    if 'ppg_last_10_games' not in X.columns:
        logging.error("'ppg_last_10_games' not found in input features. Cannot include it in predictions.")
        return pd.DataFrame()

    predictions = X[['player_id', 'ppg_last_10_games']].copy()

    # Iterate over each target to make predictions
    for target in ['10_plus', '15_plus', '20_plus']:
        ...
        # >>> DEBUG: Check if SGA is in the DataFrame for this target <<<
        sga_idx = None
        if 'player_id' in X.columns:
            sga_rows = X[X['player_id'] == '1628983'].index
            if not sga_rows.empty:
                sga_idx = sga_rows[0]  # Take the first matching row
                logging.debug(f"SGA found at index {sga_idx} for target '{target}'.")
        # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        # >>> DEBUG: If we found SGA, log his scaled features & probability <<<
        if sga_idx is not None:
            # Log the raw feature values from X
            logging.debug("=== DEBUG (make_predictions) ===")
            logging.debug(f"SGA (player_id=1628983) raw features for target '{target}':")
            sga_raw_features = X.loc[sga_idx, feature_columns].to_dict()
            logging.debug(sga_raw_features)

            # Log the scaled feature values
            sga_scaled = X_scaled[sga_idx]
            logging.debug(f"SGA scaled features (array) for target '{target}': {sga_scaled}")

            # Log the actual prediction
            sga_probability = proba[sga_idx]
            logging.debug(f"SGA predicted probability for target '{target}': {sga_probability}")
...
def generate_top_scorers(combined_data, predictions, top_n=25):
    ...
    priority_order = ['20_plus', '15_plus', '10_plus']
    ...
    for target in priority_order:
        ...
        # Ensure 'ppg_last_10_games' exists for sorting
        if 'ppg_last_10_games' not in merged.columns:
            logging.error(f"'ppg_last_10_games' column not found in merged DataFrame for target '{target}'.")
            continue

        # Filter out excluded players
        target_df = merged[~merged['player_id'].isin(excluded_player_ids)].copy()

        # Sort by predicted probability descending and then by 'ppg_last_10_games' descending
        top = target_df[['player_id', 'player_name', 'team_id', target, 'ppg_last_10_games']].sort_values(
            by=[target, 'ppg_last_10_games'], ascending=[False, False]
        ).head(top_n)

        # Rename the probability column
        top.rename(columns={target: 'predicted_probability'}, inplace=True)

        # Drop 'ppg_last_10_games' as it's no longer needed
        top = top.drop(columns=['ppg_last_10_games'])
...
def verify_high_performers(top_scorers, high_performers, players_dir, best_models):
    ...
    for player_id in high_performers:
        ...
        for target, threshold in [('20_plus', 20), ('15_plus', 15), ('10_plus', 10)]:  # Adjusted priority for verification
            if target not in best_models:
...
def main():
    try:
        ...
        for target, threshold in targets.items():
            logging.info(f'\n=== Making predictions for target: {target} (> {threshold} points) ===')
        ...
        # To ensure that all 'ppg_last_10_games' are included, merge on 'player_id'
        # Each target has its own 'predicted_probability', but 'ppg_last_10_games' is shared
        # Ensure 'ppg_last_10_games' is consistent across targets
        # Take 'ppg_last_10_games' from the first target that has it

        # For clarity, ensure 'ppg_last_10_games' is present
        # Take it from the first non-empty prediction
        ppg_column_found = False
        for target in ['20_plus', '15_plus', '10_plus']:
        ...
        if not ppg_column_found:
            logging.error("'ppg_last_10_games' not found in any predictions. Cannot proceed with Top Scorers generation.")
            return

        # Create combined_predictions with 'player_id' and 'ppg_last_10_games'
        # Assuming 'ppg_last_10_games' is the same across all targets
        # Use 'ppg_last_10_games' from the last target that has it
        # Here, we'll assume it's consistent and take it from '20_plus' if available, else '15_plus', else '10_plus'

        combined_predictions = pd.DataFrame()

        for target in ['20_plus', '15_plus', '10_plus']:
            if target in all_predictions and not all_predictions[target].isnull().all():
                # Extract 'player_id' and 'ppg_last_10_games' from predictions
                # Assuming 'ppg_last_10_games' is the same across all targets
                combined_predictions['player_id'] = X['player_id']
                combined_predictions['ppg_last_10_games'] = X['ppg_last_10_games']
                break
