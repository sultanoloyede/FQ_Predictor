...
def fetch_top_n_rpg_players(current_season, teams_df, active_player_ids, top_n=4):
    """
    ...
    try:
        ...
        # Rename 'REB' to 'AVG_REB' for clarity
        df_current_season['AVG_REB'] = df_current_season['REB']
        ...
        # Sort by team + AVG_REB, pick top_n per team
        top_players = (
            df_current_season
            .sort_values(['TEAM_ID', 'AVG_REB'], ascending=[True, False])
            .groupby('TEAM_ID')
            .head(top_n)
        )
        ...
        for team_id, count in players_per_team.items():
            ...
            logging.info(f"Team: {team_name} (ID: {team_id}) - Top {count} Active RPG Players.")
        ...
        logging.error(f"Error fetching top {top_n} active RPG players for {current_season}: {e}")
        return pd.DataFrame()
...
def main():
    ...
    top_players_df = fetch_top_n_rpg_players(current_season, teams_df, active_player_ids, top_n=4)
