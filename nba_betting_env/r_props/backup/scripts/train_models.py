...
# Removed SVC import as per instructions
...
def load_player_data(players_dir):
    ...
    for file in player_files:
        ...
        # Removed 'fga' as it's no longer needed
...
def prepare_features(combined_data, target_threshold):
    ...
    for player_id, group in grouped:
        ...
        # Compute rolling statistics for rebounds instead of points
        ...
        # Target: Whether the player had above the target threshold rebounds in the latest game
        target = 1 if latest_game['rebounds'] > target_threshold else 0
    ...
    if features:
        X = np.array(features)
        y = np.array(targets)
        logging.info(f'Prepared features and targets for {players_processed} players for target > {target_threshold} rebounds.')
    else:
        X = np.array([])
        y = np.array([])
...
def train_and_tune_model(X, y, model_name):
    ...
    # Removed SVC as per instructions
    elif model_name == 'KNeighbors':
...
def main():
    try:
        ...
        # Define targets and thresholds for rebounds
        targets = {
            '4_plus': 4,
            '6_plus': 6,
            '8_plus': 8,
        ...
        # Define models to evaluate (Removed 'SVC')
        ...
        for target_name, threshold in targets.items():
            logging.info(f'\n=== Training for Target: {target_name} (> {threshold} rebounds) ===')
