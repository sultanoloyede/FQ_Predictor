...
# Load the dataset.
data_path = 'final_training_dataset_2024.csv'
...
# Original columns_to_keep (before removal)
columns_to_keep = [
    'TEAM_ABBREVIATION',
...
# Adjust this if any of the removed columns were categorical.
le = LabelEncoder()
...
# Apply any feature transformations only on the retained columns
df['Pace_Interaction'] = df['Season_Avg_Pace_Team'] * df['Season_Avg_Pace_Opponent']
df['PPG_Interaction'] = df['Season_Avg_FG_PCT_Team'] * df['Season_Avg_FG_PCT_Opponent']  # Adjust if this was originally involving PPG
df['FG_PCT_Interaction'] = df['Season_Avg_FG_PCT_Team'] * df['Season_Avg_FG_PCT_Opponent']

df['PPG_Team_Squared'] = 0  # If you previously used PPG_Team, now it's removed. Set to 0 or remove these lines.
df['PPG_Opponent_Squared'] = 0
df['Head_to_Head_Q1_Squared'] = 0
...
# If Off_Rating_Avg and Opp_Off_Rating_Avg were used:
df['Off_Rating_Avg_Difference'] = 0
df['Off_Rating_Avg_Interaction'] = 0

# Drop features that may have little importance (if previously done)
...
model_path = 'best_nba_modelx.pkl'
scaler_path = 'scalerx.pkl'
...
if os.path.exists(model_path) and os.path.exists(scaler_path):
    ...
    print(f"XGBoost ROC-AUC Score: {roc_auc_score(y_test, y_pred_xgb):.4f}")
    
    # Prediction for the first row of the test set.
    first_test_instance = X_test.iloc[0].values.reshape(1, -1)
    first_prediction = xgb.predict(first_test_instance)
    first_prediction_proba = xgb.predict_proba(first_test_instance)
    ...
    cv_folds = 5
    xgb_cv_scores = cross_val_score(xgb_cv, X, y, cv=cv_folds, scoring='roc_auc', n_jobs=-1)
    
    print(f"XGBoost Cross-Validation ROC-AUC Scores: {xgb_cv_scores}")
    print(f"Mean ROC-AUC Score: {xgb_cv_scores.mean():.4f}")
    print(f"Standard Deviation of ROC-AUC Score: {xgb_cv_scores.std():.4f}")
    ...
    xgb_grid_search = GridSearchCV(xgb_grid, xgb_param_grid, cv=cv_folds, scoring='roc_auc', n_jobs=-1)
    xgb_grid_search.fit(X, y)
    
    print(f"Best Parameters for XGBoost: {xgb_grid_search.best_params_}")
    print(f"Best ROC-AUC Score from Grid Search for XGBoost: {xgb_grid_search.best_score_:.4f}")
...
if os.path.exists(model_path) and os.path.exists(scaler_path):
    ...
    print(f"TEAM1: {team1}")
    print(f"TEAM2: {team2}")
    print(f"Predicted Class: {'Over 52' if loaded_prediction[0] == 1 else 'Under 52'}")
