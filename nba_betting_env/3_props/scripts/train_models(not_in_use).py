...
def prepare_features(combined_data, target_threshold):
    ...
    for player_id, group in grouped:
        ...
        group['3ppg_last_10_games'] = group['3points'].rolling(window=10, min_periods=10).mean()
        group['3ppg_last_20_games'] = group['3points'].rolling(window=20, min_periods=20).mean()
        group['pct_1_plus_last_10_games'] = group['3points'].rolling(window=10, min_periods=10).apply(lambda x: (x > 1).mean())
        group['pct_2_plus_last_10_games'] = group['3points'].rolling(window=10, min_periods=10).apply(lambda x: (x > 2).mean())
        group['pct_3_plus_last_10_games'] = group['3points'].rolling(window=10, min_periods=10).apply(lambda x: (x > 3).mean())
        group['avg_score_bottom_20_percent_games'] = group['3points'].rolling(window=20, min_periods=20).apply(compute_bottom_20_avg)
        group['fg3a_avg_last_10_games'] = group['fg3a'].rolling(window=10, min_periods=10).mean()
        group['fg3a_avg_season'] = group['fg3a'].expanding(min_periods=20).mean()
        group['fg3a_avg_last_5_games'] = group['fg3a'].rolling(window=5, min_periods=5).mean()
        ...
        target = 1 if latest_game['3points'] > target_threshold else 0
    ...
    if features:
        X = np.array(features)
        y = np.array(targets)
        logging.info(f'Prepared features and targets for {players_processed} players for target > {target_threshold} 3points.')
    else:
        X = np.array([])
        y = np.array([])
...
def plot_feature_importance_or_correlation(model, X, y, feature_columns, target_name, output_dir):
    """
    ...
    # 1) If the model has feature_importances_ (e.g., tree-based models)
    if hasattr(model, 'feature_importances_'):
        ...
        plt.title(f'{target_name.upper()} - Feature Importances ({type(model).__name__})')
        plt.xlabel('Importance Score')
        plt.ylabel('Features')
    
    # 2) If the model has coef_ (e.g., LogisticRegression)
    elif hasattr(model, 'coef_'):
        # For multi-class or multi-output, we take the first row of coefficients
        ...
        plt.title(f'{target_name.upper()} - |Coefficient| ({type(model).__name__})')
        plt.xlabel('Absolute Coefficient Value')
        plt.ylabel('Features')
    
    # 3) Otherwise, plot correlation with target
    else:
        df = pd.DataFrame(X, columns=feature_columns)
        df['target'] = y
        
        corrs = df.corr()['target'].drop('target')
        corrs_abs = corrs.abs().sort_values(ascending=False)  # absolute correlation, descending
        sns.barplot(x=corrs_abs.values, y=corrs_abs.index, palette='viridis')
        plt.title(f'{target_name.upper()} - Feature Correlation with Target')
        plt.xlabel('Absolute Correlation')
        plt.ylabel('Features')
    
    plt.tight_layout()
    # Save the plot
    plot_path = os.path.join(output_dir, f"feature_importance_{target_name}.png")
    plt.savefig(plot_path)
    plt.close()
    logging.info(f"Feature importance/correlation plot saved to {plot_path}")
...
def main():
    try:
        ...
        # Ensure models and plots directory exist
        ...
        model_names = ['XGBoost', 'GradientBoosting', 'LogisticRegression', 'KNeighbors']

        # Dictionary to store best models and their scores
        best_models = {}

        for target_name, threshold in targets.items():
            logging.info(f'\n=== Training for Target: {target_name} (> {threshold} 3points) ===')

            # Prepare features and target
            X, y, feature_columns = prepare_features(combined_data, threshold)
            ...
            if overall_best_model is not None:
                ...
                # We use the final best model (overall_best_model)
                # and the unscaled X (so correlation makes sense if we fall back to correlation),
                # but one could also plot importance using the scaled X if desired.
                plot_feature_importance_or_correlation(
                    model=overall_best_model,
                    X=X,  # or X_scaled if you prefer
                    y=y,
