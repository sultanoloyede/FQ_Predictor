...
def fetch_top_n_3pg_players(current_season, teams_df, active_player_ids, top_n=4):
    """
    ...
    try:
        ...
        # Rename 'FG3M' to 'AVG_FG3M' for clarity
        df_current_season['AVG_FG3M'] = df_current_season['FG3M']
        ...
        # Sort by team + AVG_FG3M, pick top_n per team
        top_players = (
            df_current_season
            .sort_values(['TEAM_ID', 'AVG_FG3M'], ascending=[True, False])
            .groupby('TEAM_ID')
            .head(top_n)
        )
        ...
        for team_id, count in players_per_team.items():
            ...
            logging.info(f"Team: {team_name} (ID: {team_id}) - Top {count} Active 3PG Players.")
        ...
        logging.error(f"Error fetching top {top_n} active 3PG players for {current_season}: {e}")
        return pd.DataFrame()
...
def main():
    ...
    top_players_df = fetch_top_n_3pg_players(current_season, teams_df, active_player_ids, top_n=4)
