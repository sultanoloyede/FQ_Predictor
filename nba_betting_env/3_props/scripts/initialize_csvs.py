...
# Function to Fetch Top N 3ppg Players per Team
def fetch_top_n_3ppg_players(current_season, last_completed_season, teams_df, top_n=4):
    try:
        ...
        # Remove duplicate player-season entries by aggregating FG3M (sum or mean)
        # Here, we'll take the mean FG3M across seasons
        combined_df = combined_df.groupby(['PLAYER_ID', 'PLAYER_NAME', 'TEAM_ID'], as_index=False).agg({'FG3M': 'mean'}).rename(columns={'FG3M': 'AVG_FG3M'})
        ...
        # Sort by TEAM_ID and AVG_FG3M to get top N 3ppg players per team
        top_players = combined_df.sort_values(['TEAM_ID', 'AVG_FG3M'], ascending=[True, False]).groupby('TEAM_ID').head(top_n)
        ...
        for team_id, count in players_per_team.items():
            ...
            logging.info(f"Team: {team_name} (ID: {team_id}) - Top {count} 3ppg Players Fetched.")
        ...
        if teams_missing:
            ...
            logging.info("All teams have their top 3ppg players fetched.")

        return top_players
    except Exception as e:
        logging.error(f"Error fetching top {top_n} 3ppg players: {e}")
        return pd.DataFrame()
...
def initialize_player_csv(player, seasons_to_fetch):
    ...
    relevant_columns = ['GAME_ID', 'GAME_DATE', 'FG3M', 'FG3A', 'MIN', 'PF']
...
def main():
    ...
    # Fetch Current Top 4 3ppg Players
    top_players_df = fetch_top_n_3ppg_players(current_season, last_completed_season, teams_df, top_n=4)
