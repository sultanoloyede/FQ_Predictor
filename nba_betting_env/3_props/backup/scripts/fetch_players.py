...
# Function to Fetch Top N 3p Players per Team
def fetch_top_n_3p_players(current_season, last_completed_season, teams_df, top_n=4):
    try:
        ...
        # Sort by TEAM_ID and AVG_FG3M to get top N 3p players per team
        ...
        for team_id, count in players_per_team.items():
            ...
            logging.info(f"Team: {team_name} (ID: {team_id}) - Top {count} 3p Players Fetched.")
        ...
        if teams_missing:
            ...
            logging.info("All teams have their top 3p players fetched.")

        return top_players
    except Exception as e:
        logging.error(f"Error fetching top {top_n} 3p players: {e}")
        return pd.DataFrame()
...
def main():
    ...
    # Fetch Current Top 4 3p Players
    top_players_df = fetch_top_n_3p_players(current_season, last_completed_season, teams_df, top_n=4)
