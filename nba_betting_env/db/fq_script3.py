...
def calculate_ppg_and_off_rating(training_dataset_df):
    ...
    # Calculate cumulative Offensive Rating excluding the current game
    training_dataset_df['Cumulative_Opponent_Off_Rating_Previous'] = training_dataset_df.groupby(['TEAM_ID', 'SEASON'])['Opponent_Off_Rating'].cumsum().shift(1)
    ...
    # Calculate Offensive Rating up to the previous game
    training_dataset_df['Opponent_Off_Rating_Avg'] = training_dataset_df['Cumulative_Opponent_Off_Rating_Previous'] / (training_dataset_df['Game_Number'] - 1)
    ...
    training_dataset_df.loc[first_games_mask, 'Opponent_Off_Rating_Avg'] = training_dataset_df.loc[first_games_mask, 'Opponent_Off_Rating']

    print("\nCalculated PPG, Opponent PPG, and Offensive Rating up to the previous game.")

    # Drop intermediate columns
    training_dataset_df.drop(['Cumulative_Points_Previous', 'Cumulative_Opp_Points_Previous', 'Cumulative_Off_Rating_Previous', 'Game_Number', 'Cumulative_Opponent_Off_Rating_Previous'], axis=1, inplace=True)
...
for idx, row in unique_games_teams.iterrows():
    ...
    # Fetch Off_Rating for team
    if (game_id, team_id) not in off_rating_cache:
        off_rating_cache[(game_id, team_id)] = fetch_off_rating(game_id, opponent_id, off_rating_cache)
...
# Merge Opponent PTS into training_dataset_df
opponent_pts_df = pts_df.rename(columns={'TEAM_ID': 'OPPONENT_TEAM_ID', 'PTS': 'OPPONENT_OFF_RATING'})
training_dataset_df = training_dataset_df.merge(
    opponent_pts_df[['GAME_ID', 'OPPONENT_TEAM_ID', 'OPPONENT_OFF_RATING']],
    on=['GAME_ID', 'OPPONENT_TEAM_ID'],
    how='left'
)
...
training_dataset_df['Opponent_Off_Rating'] = pd.to_numeric(training_dataset_df['Off_Rating'], errors='coerce')
