...
if len(sys.argv) > 1:
    ...
    while(count < 1):
        ...
        new_game_id = str(22400301).zfill(10)
        ...
        def fetch_latest_off(csv_filename, team_id):
            """
        ...
        def calc_off(csv_filename, team_id):
            df = pd.read_csv(csv_filename, dtype={'TEAM_ID': str}, parse_dates=['GAME_DATE'])
            team_id = str(team_id)
            team_games = df[df['TEAM_ID'] == team_id]
            if team_games.empty:
            ...
            off = ((season_avg_pace * num_of_games(csv_filename, team_id)) + fetch_latest_off(csv_file, team_id)) / (num_of_games(csv_filename, team_id) + 1)
            return off
        ...
        def append_game_data(game_id, csv_filename='final_training_dataset.csv'):
            ...
            try:
                ...
                if '@' in matchup:
                    ...
                    home_away = 'Away' if team_abbrev == away_team else 'Home'
                    mtch = home_team + " vs. " + away_team
                    alt_HA = 'Home' if team_abbrev == away_team else 'Away'
                elif 'vs' in matchup:
                    # Format: HomeTeam vs AwayTeam
                    teams = matchup.split('vs')
                    home_team = teams[0].strip()
                    away_team = teams[1].strip()
                    home_away = 'Home' if team_abbrev == home_team else 'Away'
                    mtch = away_team + " @ " + home_team
                    alt_HA = 'Away' if team_abbrev == away_team else 'Home'
                else:
                    home_away = 'Unknown'
                ...
                recent_array.append(fetch_pts_qtr1(game_id, team['TEAM_ID']))
                print(recent_array)
                recent_array_opp = get_recent_trend(csv_file, opponent['TEAM_ID'], game_date)
                recent_array_opp.append(fetch_pts_qtr1(game_id, opponent['TEAM_ID']))
                ...
                off_rating = fetch_latest_off(csv_file, team['TEAM_ID'])
                off_rating_avg = calc_off(csv_filename, team['TEAM_ID'])
                opp_off_rating_avg = calc_off(csv_filename, opponent['TEAM_ID'])
